...
function isLegal(square, moveFile, moveRank, observer) {
    ...
    // if(bool && king.isAttacked)
    // {
        // king.isChecked();
    // }

//    if(checkMate(king,color) && king.isChecked() )
//            return "CheckMate";
...
function moveBishop(square,moveFile, moveRank,observer,color){
    ...
    if(Math.abs(moveFile-square.file())
...
function moveRook(square,moveFile, moveRank,observer){
    ...
    if(moveFile == square.file() || moveRank == square.rank())
    {
        observer.board.move(square, moveFile,moveRank);
...
function moveKing(square,moveFile,moveRank,observer,color) {
    ...
    if(Math.abs(moveFile - square.file()) <= 1
        && Math.abs(moveRank - square.rank()) <=1 ) {
        observer.Board.move(square,moveFile,moveRank);
        square.getPiece().moved();
        king.isUnchecked();
